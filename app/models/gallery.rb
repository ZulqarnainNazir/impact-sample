class Gallery < ActiveRecord::Base
  ...
  def share_callback_url
    if self.business.webhost_primary? && !self.business.is_on_engage_plan?
      ...
      "http://#{ENV['LISTING_HOST']}#{self.business.generate_listing_path}/#{self.slug}?content=gallery"
    end
  end
  ...
  def og_image_url
    gallery_images.first.try(:gallery_image).try(:attachment_full_url, :jumbo)
  end
