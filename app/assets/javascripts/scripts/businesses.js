$(document).on('change', 'form.community_selector select', function(e) {
  var form = $(this).closest('form');
  form.find('select').prop('disabled', true);
  form.find('i').remove();
  form.append($('<i class="fa fa-spin fa-spinner"></i>'));
  // alert(form.attr('action'));
  // alert(form.find('select').prop('value'));
  var jqxhr = $.ajax({
    url: form.attr('action'),
    type: 'PUT',
    data: { community_id: form.find('select').prop('value') },
    dataType: 'json'
  });
  // console.log(jqxhr);
