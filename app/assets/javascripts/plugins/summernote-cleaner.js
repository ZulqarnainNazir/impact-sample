        ...
        'summernote.paste':function (we, e) {
          if (options.cleaner.action == 'both' || options.cleaner.action == 'paste') {
            ...
            if (msie) {
              ...
              var htmlText = options.cleaner.keepHtml && e.originalEvent.clipboardData.getData('text/html')
            ...
            function pasteOrCode(t) {
                ...
                if ($note.summernote('isEmpty')) {
                    ...
                    $note.summernote('pasteHTML', t);
                }
            }

            if (text) {
              if (msie || ffox)
                setTimeout(pasteOrCode(cleanText(text, options.cleaner.newline)), 1);
              else
                pasteOrCode(cleanText(text, options.cleaner.newline));
              if ($('.note-status-output').length > 0)
