...
(function() {
  ...
  ERD = (function() {
    function ERD(name1, elem, edges1) {
      var j, len, model, models;
      this.name = name1;
      this.elem = elem;
      this.edges = edges1;
      this.handle_close_migration_click = bind(this.handle_close_migration_click, this);
      this.handle_open_migration_click = bind(this.handle_open_migration_click, this);
      this.handle_new_model_add_column_click = bind(this.handle_new_model_add_column_click, this);
      this.handle_remove_model_click = bind(this.handle_remove_model_click, this);
      this.handle_text_elem_click = bind(this.handle_text_elem_click, this);
      this.handle_cancel_click = bind(this.handle_cancel_click, this);
      this.handle_add_column_click = bind(this.handle_add_column_click, this);
      this.handle_rename_model = bind(this.handle_rename_model, this);
      this.handle_rename_column = bind(this.handle_rename_column, this);
      this.handle_change_column_type = bind(this.handle_change_column_type, this);
      this.handle_add_column = bind(this.handle_add_column, this);
      this.handle_save = bind(this.handle_save, this);
      this.handle_drag = bind(this.handle_drag, this);
      this.connect_arrows = bind(this.connect_arrows, this);
    ...
    ERD.prototype.upsert_change = function(action, model, column, from, to) {
      ...
      rows = (function() {
        ...
        ref = $('#changes > tbody > tr');
        results = [];
        for (j = 0, len = ref.length; j < len; j++) {
          tr = ref[j];
          results.push($(tr).find('td'));
        }
        return results;
      })();
      existing = null;
    ...
    ERD.prototype.positions = function(div) {
      ...
      ref = [parseFloat(div.css('left')), parseFloat(div.css('width')), parseFloat(div.css('top')), parseFloat(div.css('height'))], left = ref[0], width = ref[1], top = ref[2], height = ref[3];
      return {
        left: left,
        right: left + width,
        top: top,
    ...
    ERD.prototype.connect_arrow = function(edge, from_elem, to_elem) {
      var a, b, from, j, len, path, rect, ref, ref1, ref2, ref3, ref4, ref5, ref6, to, x2y, y2x;
      ...
      if (from.center.x > to.center.x) {
        ref = [from.left, x2y(from.left)], from.vertex.x = ref[0], from.vertex.y = ref[1];
        ref1 = [to.right, x2y(to.right)], to.vertex.x = ref1[0], to.vertex.y = ref1[1];
      } else {
        ref2 = [from.right, x2y(from.right)], from.vertex.x = ref2[0], from.vertex.y = ref2[1];
        ref3 = [to.left, x2y(to.left)], to.vertex.x = ref3[0], to.vertex.y = ref3[1];
      }
      ref4 = [from, to];
      for (j = 0, len = ref4.length; j < len; j++) {
        rect = ref4[j];
        if (rect.vertex.y < rect.top) {
          ref5 = [y2x(rect.top), rect.top, 'v'], rect.vertex.x = ref5[0], rect.vertex.y = ref5[1], rect.vertex.direction = ref5[2];
        } else if (rect.vertex.y > rect.bottom) {
          ref6 = [y2x(rect.bottom), rect.bottom, 'v'], rect.vertex.x = ref6[0], rect.vertex.y = ref6[1], rect.vertex.direction = ref6[2];
        } else {
