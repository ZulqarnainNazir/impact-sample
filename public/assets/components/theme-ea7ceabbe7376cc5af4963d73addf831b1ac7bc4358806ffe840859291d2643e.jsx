(function() {
  ...
  Theme = React.createClass({
    ...
    getInitialState: function() {
      return {
        wrapContainer: this.props.defaultWrapContainer === 'true' ? true : false,
    ...
    componentDidMount: function() {
      ...
      $(document).on('click', '.logo-size-picker', function(e) {
        e.preventDefault();
        return $(e.target).closest('.logo-size').find('input[type="radio"]').prop('checked', true);
      });
      $(document).on('change', '#header_block_attributes_navbar_location', this.showLogoFixedOption);
      return $(document).on('click', '[name="header_block_attributes[logo_vertical_position]"]', this.showLogoFixedOption);
    },
    showLogoFixedOption: function() {
      if ($('#header_block_attributes_navbar_location option:selected').val() === 'fixed' && $('[name="header_block_attributes[logo_vertical_position]"]:checked').val() === "above") {
        return $("#header_block_attributes_logo_bar_fixed_wrapper").show();
      } else {
        return $("#header_block_attributes_logo_bar_fixed_wrapper").hide();
      }
    },
    setupMinicolors: function() {
      var minicolorOptions;
      minicolorOptions = {
      ...
      return $('#footer_block_attributes_link_color').minicolors(minicolorOptions);
    },
    resetColors: function() {
      ...
      return this.setState(React.addons.update(this.state, changes), this.setupMinicolors);
    },
    setColorStates: function() {
    ...
    loadColors: function() {
      ...
      return $(image).load(this.loadImageColors.bind(null, image));
    },
    ...
    loadImageColors: function(image) {
      var canvas, changes, colors, context, thief;
    ...
    toggleContainerWrapper: function() {
      return this.setState({
        wrapContainer: !this.state.wrapContainer
      });
    },
    webpageContainerClass: function() {
      if (this.state.wrapContainer) {
    ...
    renderLoadColorsButton: function() {
      if (this.state.logo) {
        return <span onClick={this.loadColors} className="btn btn-default">Load Colors from Logo Image</span>;
      }
    },
    editHeader: function(event) {
      event.preventDefault();
      $('#header_modal').modal('show');
      $('#header_block_attributes_style').val(this.state.headerBlock.style || 'light');
      $('#header_block_attributes_navbar_location').val(this.state.headerBlock.navbar_location || 'default');
      $('#header_block_attributes_logo_bar_fixed').val(this.state.headerBlock.logo_bar_fixed || 'default');
      $('input[name="header_block_attributes[logo_horizontal_position]"][value="' + (this.state.headerBlock.logo_horizontal_position || 'left') + '"]').prop('checked', true);
      $('input[name="header_block_attributes[logo_vertical_position]"][value="' + (this.state.headerBlock.logo_vertical_position || 'inside') + '"]').prop('checked', true);
      $('input[name="header_block_attributes[navigation_horizontal_position]"][value="' + (this.state.headerBlock.navigation_horizontal_position || 'right') + '"]').prop('checked', true);
      $('input[name="header_block_attributes[contact_position]"][value="' + (this.state.headerBlock.contact_position || 'none') + '"]').prop('checked', true);
      $('input[name="header_block_attributes[social_enabled]"][value="' + (this.state.headerBlock.social_enabled || 'hidden') + '"]').prop('checked', true);
      return this.showLogoFixedOption();
    },
    resetHeader: function() {
      ...
      return $('input[name="header_block_attributes[social_enabled]"][value="hidden"]').prop('checked', true);
    },
    updateHeader: function() {
      ...
      return this.setState({
        headerBlock: React.addons.update(this.state.headerBlock, changes)
      });
    },
    editLogo: function(event) {
      event.preventDefault();
      ...
      switch (parseInt(this.state.headerBlock.logo_height)) {
        case 40:
          return $('#logo_size_40').prop('checked', true);
        case 60:
          return $('#logo_size_60').prop('checked', true);
        case 125:
          return $('#logo_size_125').prop('checked', true);
        case 200:
          return $('#logo_size_200').prop('checked', true);
        default:
          $('#logo_size_custom').prop('checked', true);
          return $('#logo_size_custom_value').val(this.state.headerBlock.logo_height);
      }
    },
    resetLogo: function() {
      $('#header_block_attributes_logo_height').val('');
      $('#logo_size_custom_value').val('');
      return $('input[name="logo_size"]').val('');
    },
    updateLogo: function() {
      var changes, logo_height;
      logo_height = $('#logo_size_custom').is(':checked') ? $('#logo_size_custom_value').val() : $('input[name="logo_size"]:checked').val();
      changes = {
        $merge: {
    ...
    prevFooterTheme: function(event) {
      ...
      return this.setState({
        footerBlock: React.addons.update(this.state.footerBlock, {
          $merge: {
            theme: theme
          }
        })
    ...
    nextFooterTheme: function(event) {
      var availableThemes, currentTheme, currentThemeIndex, theme;
      event.preventDefault();
      availableThemes = ['simple', 'simple_full_width', 'columns', 'layers'];
      currentTheme = this.state.footerBlock.theme;
